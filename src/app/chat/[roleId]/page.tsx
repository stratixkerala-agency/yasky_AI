    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="lg:pl-64">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 sm:gap-4">
                <button 
                  ...
                  className="lg:hidden flex items-center justify-center w-10 h-10 bg-zinc-800/50 hover:bg-zinc-700/50 rounded-xl transition-colors"
                >
        ...
        <main className="relative flex-1 overflow-hidden">
          <div className="max-w-5xl mx-auto h-[calc(100vh-140px)] sm:h-[calc(100vh-130px)] flex flex-col">
            {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center px-4">
                <div className={cn('w-14 sm:w-16 h-14 sm:h-16 rounded-2xl bg-gradient-to-br flex items-center justify-center mb-4 sm:mb-6', gradientClass)}>
                  {renderIcon(role.icon)}
                </div>
                <h2 className="text-xl sm:text-2xl font-semibold text-white mb-2">Start a conversation</h2>
                <p className="text-zinc-400 text-center max-w-md mb-6 sm:mb-8 px-4">
                  {role.description}
                </p>
                ...
                {messages.map((message) => (
                  ...
                  >
                    ...
                    <div className={cn(
                        ...
                        <div className="prose prose-invert prose-sm max-w-none">
                          <ReactMarkdown
                            components={{
                              ...
                              h1: ({ children }) => <h1 className="text-lg sm:text-xl font-bold text-white mt-4 mb-2">{children}</h1>,
                              h2: ({ children }) => <h2 className="text-base sm:text-lg font-semibold text-white mt-4 mb-2">{children}</h2>,
                              h3: ({ children }) => <h3 className="text-sm sm:text-md font-semibold text-white mt-3 mb-2">{children}</h3>,
                              ...
                              code: ({ children }) => <code className="bg-zinc-800 px-1.5 py-0.5 rounded text-purple-400 text-xs sm:text-sm">{children}</code>,
                              pre: ({ children }) => <pre className="bg-zinc-800 p-3 rounded-lg overflow-x-auto my-2 text-sm">{children}</pre>,
                            }}
                          >
                            {message.content}
                          </ReactMarkdown>
                      ...
                      {message.role === 'user' && (
                        <p className="text-zinc-200 whitespace-pre-wrap text-sm sm:text-base">{message.content}</p>
                      )}
                      <div className={cn(
            ...
            <form onSubmit={handleSubmit} className="flex items-end gap-2 sm:gap-3">
              <div className="flex-1 relative">
                <textarea
                  ...
                  rows={1}
                  className="w-full px-4 py-3 bg-zinc-800/50 border border-zinc-700 rounded-xl text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none max-h-32 text-sm sm:text-base"
              ...
              <button
                ...
                className={cn(
                  'flex items-center justify-center w-11 sm:w-12 h-11 sm:h-12 rounded-xl transition-all duration-200',
